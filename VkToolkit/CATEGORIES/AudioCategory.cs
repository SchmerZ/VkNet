...
namespace VkToolkit.Categories
{
    public class AudioCategory
    {
        ...
        private IEnumerable<Audio> InternalGet(string paramId, long id, out User user, long? albumId = null, IEnumerable<long> aids = null, bool? needUser = null, int? count = null, int? offset = null)
        {
            ...
            var url = _vk.GetApiUrl("audio.get", values);
            var data = _vk.HttpClient.Get<List<Audio>>(url);

            _vk.IfErrorThrowException(data.Error);

            user = null;

            if (needUser.HasValue && needUser.Value)
                data.Resource.RemoveAt(0);
        ...
        public IEnumerable<Audio> GetById(IEnumerable<string> audios)
        {
            ...
            var values = new Dictionary<string, string>();
            values.Add("audios", audios.GetEnumerationAsString());

            var url = _vk.GetApiUrl("audio.getById", values);
            var data = _vk.HttpClient.Get<List<Audio>>(url);
        ...
        public int GetCount(long ownerId)
        {
            ...
            var url = _vk.GetApiUrl("audio.getCount", values);
            var json = _vk.Browser.GetJson(url);
        ...
        public Lyrics GetLyrics(long id)
        {
            ...
            var url = _vk.GetApiUrl("audio.getLyrics", values);
            var json = _vk.Browser.GetJson(url);
